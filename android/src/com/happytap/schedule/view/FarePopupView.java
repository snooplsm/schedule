...
public class FarePopupView {
	...
	public FarePopupView(View anchor, Rect frame, final Map<String,Double> fares, String date) {
		if (anchor == null)
		...
		final List<String> keys = new ArrayList<String>(fares.keySet());
		list.setAdapter(new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_fare_item, null);
				TextView key = (TextView)convertView.findViewById(android.R.id.text1);
				TextView val = (TextView)convertView.findViewById(android.R.id.text2);
				String k = getItem(position);
				Double d = fares.get(k);
				key.setText(k+":");
				val.setText(LoadScheduleActivity.df.format(d));
				return convertView;
			}
		});
